...
class BiLSTM_CRF(nn.Module):
    ...
    def init_hidden(self):
        return (torch.randn(2, 1, self.hidden_dim // 2).to("cuda:0"),
                torch.randn(2, 1, self.hidden_dim // 2).to("cuda:0"))

    def _forward_alg(self, feats):
        # Do the forward algorithm to compute the partition function
        init_alphas = torch.full((1, self.tagset_size), -10000.).to("cuda:0")
    ...
    def _score_sentence(self, feats, tags):
        ...
        tags = torch.cat([torch.tensor([self.tag_to_ix[config["start_tag"]]], dtype=torch.long).to("cuda:0"), tags])
    ...
    def _viterbi_decode(self, feats):
        ...
        init_vvars = torch.full((1, self.tagset_size), -10000.).to("cuda:0")
